...
struct listWORD *advancePoccess(char *codeSTRING){
    ...
    struct listWORD *pS = poccessSeparator(codeSTRING)
    return poccessMacro(pS);
}
...
float detectRepetition(char *codeSTRING1, char *codeSTRING2){
    ...
    struct listWORD *headA = advancePoccess(codeSTRING1);
    struct listWORD *tailA = NULL;
    struct listWORD *headB = advancePoccess(codeSTRING2);
    struct listWORD *tailB = NULL;
    headA = headA->next;
    headB = headB->next;
    headA = a;
